...
export default function XiaoHeAI() {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      setMessages(prev => [...prev, { role: 'ai', content: "连接失败，请检查设置。" }]);
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', backgroundColor: '#f8fafd' }}>
      {/* 顶部标题 */}
      <div style={{ padding: '15px', textAlign: 'center', fontSize: '24px', fontWeight: 'bold', color: '#0b57d0' }}>小何 AI 情绪盒</div>
        ...
        <div style={{ maxWidth: '850px', margin: '0 auto', padding: '20px 0' }}>
          {messages.length === 0 && (
            <div style={{ marginTop: '15vh', textAlign: 'center' }}>
              <h1 style={{ fontSize: '45px', color: '#1f1f1f', marginBottom: '15px' }}>你好，我是小何。</h1>
              <p style={{ fontSize: '22px', color: '#444746' }}>想聊点什么？（当前模式：{mode}）</p>
            </div>
          )}
          {messages.map((msg, i) => (
            <div key={i} style={{ marginBottom: '35px', display: 'flex', flexDirection: 'column', alignItems: msg.role === 'user' ? 'flex-end' : 'flex-start' }}>
              <div style={{ 
                padding: '18px 28px', borderRadius: '28px', fontSize: '22px', lineHeight: '1.5',
                backgroundColor: msg.role === 'user' ? '#e9eef6' : 'transparent', color: '#1f1f1f', maxWidth: '85%'
              }}>
                {msg.fileName && <div style={{ fontSize: '12px', color: '#0b57d0', marginBottom: '8px' }}>📎 已上传: {msg.fileName}</div>}
                {msg.content}
              </div>
            </div>
          ))}
      ...
      <div style={{ padding: '15px 5%', borderTop: '1px solid #f0f4f9', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
        <div style={{ width: '100%', maxWidth: '850px', backgroundColor: '#f0f4f9', borderRadius: '35px', padding: '5px 20px', display: 'flex', alignItems: 'center', boxShadow: '0 2px 10px rgba(0,0,0,0.05)' }}>
          <select value={mode} onChange={(e) => setMode(e.target.value)} style={{ background: 'none', border: 'none', fontWeight: 'bold', fontSize: '16px', outline: 'none', cursor: 'pointer' }}>
          ...
          <input value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleSend()} placeholder="问问小何..." style={{ flex: 1, border: 'none', background: 'none', padding: '15px', outline: 'none', fontSize: '22px' }} />
          <button onClick={startSpeech} style={{ border: 'none', background: 'none', fontSize: '28px', cursor: 'pointer', marginRight: '10px' }}>🎙️</button>
        ...
        <div style={{ marginTop: '15px', textAlign: 'center' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'center', gap: '25px', marginTop: '10px' }}>
            <img src="https://youke3.picui.cn/s1/2026/01/12/6963e20a9d019.png" style={{ width: '125px', borderRadius: '15px', boxShadow: '0 5px 15px rgba(0,0,0,0.1)' }} />
            <img src="https://youke3.picui.cn/s1/2026/01/12/6963e23771e4a.png" style={{ width: '125px', borderRadius: '15px', boxShadow: '0 5px 15px rgba(0,0,0,0.1)' }} />
          </div>
        </div>
      </div>
    </div>
